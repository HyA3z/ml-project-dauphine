"""
╔══════════════════════════════════════════════════════════════════════════════╗
║           TIKTOK VIRALITY — STACKING v9 : CONTENT-DEEP                    ║
║           Speech Feature Engineering + Hybrid Model                     ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  What's new in v9 vs v8:                                                      ║
║  • has_speech (flag 0/1)                                                   ║
║  • Hook features: is_question, is_list, is_urgency (first 5 words)     ║
║  • Speech rate (words/s) — dynamism proxy                                  ║
║  • Composite hook score                                                     ║
║  • Lexical sentiment (positive_count, negative_count, sentiment_ratio)    ║
║  • LSA 30 components (TF-IDF + SVD, NON_VERBAL token for silent videos)     ║
║  • A/B Test: "with speech" vs "without speech" segment                       ║
║  • Cross-features speech x momentum                                        ║
║  Record to beat: R² = 0.3332 (global) | R² = 0.4368 (Comedy)           ║
║  Goal v9: R² >= 0.45 on the spoken segment                             ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
# CONSTANTS
# ─────────────────────────────────────────────────────────────────────────────
RECORD_R2_GLOBAL  = 0.3332
RECORD_R2_COMEDY  = 0.4368
RECORD_MAE        = 0.2922
TARGET            = "target_log"
N_OPTUNA_TRIALS   = 40
N_OOF_FOLDS       = 5
TOP_K_FEATURES    = 45          # +5 vs v8 pour absorber les speech features
TOP_K_FEATURES_SPEECH = 35      # pour le sous-modèle "avec parole"
N_LSA_COMPONENTS  = 30          # LSA components (TF-IDF réduit)
MIN_NICHE_SAMPLES = 80
SMOOTH_K          = 5

print("=" * 72)
print("  STACKING v9 — CONTENT-DEEP (Speech Features)")
print("=" * 72)
...
# ─────────────────────────────────────────────────────────────────────────────
# 5. TF-IDF + LSA (30 components)
# ─────────────────────────────────────────────────────────────────────────────
print("\n" + "=" * 72)
print(f"  [PHASE 4] TF-IDF + LSA ({N_LSA_COMPONENTS} composantes)")
print("=" * 72)

# ── Fit on train only (anti-leakage) ────────────────────────────────
TRAIN_RANK_MIN, TRAIN_RANK_MAX = 11, 26
train_mask_tfidf = df["video_rank"].between(TRAIN_RANK_MIN, TRAIN_RANK_MAX)
...
print(f"  Fitting TF-IDF on {train_mask_tfidf.sum():,} training videos...")
tfidf_train = tfidf.fit_transform(df.loc[train_mask_tfidf, "subtitles_for_tfidf"])
tfidf_full  = tfidf.transform(df["subtitles_for_tfidf"])

svd = TruncatedSVD(n_components=N_LSA_COMPONENTS, random_state=42)
lsa_train = svd.fit_transform(tfidf_train)
lsa_full  = svd.transform(tfidf_full)

explained_var = svd.explained_variance_ratio_.sum()
print(f"   Explained variance ({N_LSA_COMPONENTS} LSA components) : {explained_var:.1%}")

LSA_COLS = [f"lsa_{i}" for i in range(N_LSA_COMPONENTS)]
lsa_df   = pd.DataFrame(lsa_full, columns=LSA_COLS, index=df.index)
df       = pd.concat([df, lsa_df], axis=1)

# Top termes LSA-0 (composante principale)
feature_names = tfidf.get_feature_names_out()
top_terms_lsa0 = sorted(
    zip(svd.components_[0], feature_names),
    key=lambda x: abs(x[0]), reverse=True
)[:15]
print("\n  Top 15 terms of LSA-0 component (maximum variance) :")
for score, term in top_terms_lsa0:
    tag = "" if term == "non_verbal" else ""
    bar = "█" * int(abs(score) * 300)
    print(f"    {tag} {term:<25} {score:+.4f} {bar}")

# ─────────────────────────────────────────────────────────────────────────────
# 6. FULL FEATURE SET v9
# ─────────────────────────────────────────────────────────────────────────────
STATIC_FEATURES = [
...
ALL_FEATURES_V9 = (
    ...
    NLP_COLS + NICHE_COLS + SPEECH_FEATURES + LSA_COLS
)

print(f"\n   v9 total feature set : {len(ALL_FEATURES_V9)} features")
print(f"     of which Speech Features  : {len(SPEECH_FEATURES)}")
print(f"     of which LSA              : {len(LSA_COLS)}")
...
ALL_FEATURES_V9_FINAL = ALL_FEATURES_V9 + ["creator_target_mean"]
...
X_train_full = df.loc[train_mask, ALL_FEATURES_V9_FINAL].astype(float)
y_train      = df.loc[train_mask, TARGET]
X_val_full   = df.loc[val_mask,   ALL_FEATURES_V9_FINAL].astype(float)
y_val        = df.loc[val_mask,   TARGET]
X_test_full  = df.loc[test_mask,  ALL_FEATURES_V9_FINAL].astype(float)
...
for rank, (feat, imp) in enumerate(fi_series.head(TOP_K_FEATURES).items(), 1):
    ...
    elif feat in LSA_COLS:            tag = " ←  LSA"; lsa_in_top += 1
...
print(f"   LSA features in top {TOP_K_FEATURES}    : {lsa_in_top}")
...
BASE_FEATURES_AB = [f for f in SELECTED_FEATURES
                    if f not in LSA_COLS and f not in SPEECH_FEATURES]
...
SPEECH_SELECTED = [f for f in SELECTED_FEATURES
                   if f in SPEECH_FEATURES or f in LSA_COLS]
...
# ─────────────────────────────────────────────────────────────────────────────
# 11. GLOBAL STACKING v9 (XGBoost + LightGBM + CatBoost + Ridge L2)
# ─────────────────────────────────────────────────────────────────────────────
print("\n" + "=" * 72)
print("  [PHASE 5] GLOBAL STACKING v9 (full features)")
print("=" * 72)
...
# ── XGBoost ────────────────────────────────────────────────────────────────
print(f"\n    XGBoost v9 ({N_OPTUNA_TRIALS} Optuna trials)...")
...
global_results_v9["XGBoost"] = {"r2": r2_xgb, "mae": mae_xgb}

# ── LightGBM ───────────────────────────────────────────────────────────────
print(f"\n    LightGBM v9 ({N_OPTUNA_TRIALS} Optuna trials)...")
...
global_results_v9["LightGBM"] = {"r2": r2_lgbm, "mae": mae_lgbm}

# ── CatBoost ───────────────────────────────────────────────────────────────
print(f"\n    CatBoost v9 ({N_OPTUNA_TRIALS} Optuna trials)...")
...
global_results_v9["CatBoost"] = {"r2": r2_cat, "mae": mae_cat}
...
delta_v9   = r2_l2 - RECORD_R2_GLOBAL
print(f"  R²={r2_l2:.4f} | MAE={mae_l2:.4f}  {'' if r2_l2 > RECORD_R2_GLOBAL else ''}  (Delta vs v8: {delta_v9:+.4f}) | {time.time()-t0:.0f}s")
global_results_v9["Stacking-L2"] = {"r2": r2_l2, "mae": mae_l2}
...
# ─────────────────────────────────────────────────────────────────────────────
print("\n" + "=" * 72)
print("  FINAL SUMMARY — v9 vs v8")
print("=" * 72)

print(f"\n  {'Model':<30} {'R²':>8} {'Delta v8':>9} {'MAE':>8}  Status")
print(f"  {'─'*65}")
for name, res in global_results_v9.items():
    dr2  = res["r2"] - RECORD_R2_GLOBAL
    icon = "" if res["r2"] == max(r["r2"] for r in global_results_v9.values()) else "  "
    goal = ""
...
# 13. VISUALIZATION v9
# ─────────────────────────────────────────────────────────────────────────────
print("\n   Generating v9 dashboard...")

fig = plt.figure(figsize=(22, 16))
fig.patch.set_facecolor("#0d0d0d")
fig.suptitle(
    "Stacking v9 — Content-Deep Analysis | Speech Features x TikTok Virality",
...
models_compare = {
    "XGBoost v9":    global_results_v9["XGBoost"]["r2"],
    "LightGBM v9":   global_results_v9["LightGBM"]["r2"],
    "CatBoost v9":   global_results_v9["CatBoost"]["r2"],
    "Stacking-L2 v9":global_results_v9["Stacking-L2"]["r2"],
...
ax3.set_title("R² v9 vs v8 records", color=P["text"], fontweight="bold")
...
for feat in fi_top20.index:
    ...
    elif feat in LSA_COLS:         fi_colors.append(P["lsa"])
...
ax6.set_title("Top 20 Features\n Green=Speech Purple=LSA Blue=Momentum Orange=Cross", color=P["text"], fontweight="bold", fontsize=9)
...
# ── Panel 8 : Progression R² v1 → v9 ────────────────────────────────────────
ax8 = fig.add_subplot(gs[2, 2])
ax8.set_facecolor(P["ax_bg"])
for sp in ax8.spines.values(): sp.set_color(P["grid"])

best_v9    = max(r["r2"] for r in global_results_v9.values())
versions   = ["v5\n(base)", "v6\n(momentum)", "v7\n(NLP+CV)", "v8\n(niches)", "v9\n(speech)"]
r2_history = [0.18, 0.24, 0.3332, 0.3332, best_v9]
bar_colors = ["#666","#888","#aaa","#ff4d6d", P["gold"] if best_v9 >= 0.45 else P["speech"]]
...
plt.savefig("stacking_v9_content_deep.png", dpi=150, bbox_inches="tight",
            facecolor=fig.get_facecolor())
print("   Dashboard saved -> stacking_v9_content_deep.png")
plt.close()
...
results_df = pd.DataFrame([
    {"version": "v9", "model": k, "r2": v["r2"], "mae": v["mae"],
     "delta_v8": v["r2"] - RECORD_R2_GLOBAL,
     "beats_record": v["r2"] > RECORD_R2_GLOBAL,
     "reaches_040": v["r2"] >= 0.40,
     "reaches_045": v["r2"] >= 0.45}
    for k, v in global_results_v9.items()
] + [
    {"version": "v8_record", "model": "global", "r2": RECORD_R2_GLOBAL, "mae": RECORD_MAE,
     "delta_v8": 0, "beats_record": False, "reaches_040": False, "reaches_045": False},
    {"version": "v8_record", "model": "Comedy", "r2": RECORD_R2_COMEDY, "mae": np.nan,
     "delta_v8": RECORD_R2_COMEDY - RECORD_R2_GLOBAL, "beats_record": True, "reaches_040": True, "reaches_045": False}
])
results_df.to_csv("results_v9.csv", index=False)
print("   Results exported -> results_v9.csv")

# Full feature importance
fi_series.to_frame("importance").reset_index().rename(columns={"index": "feature"}).to_csv(
    "feature_importance_v9.csv", index=False
)
print("   Feature importance -> feature_importance_v9.csv")

# Enriched dataset (speech features + niche)
...
df[export_cols].to_csv("df_v9_enriched.csv", index=False)
print("   Enriched dataset -> df_v9_enriched.csv")

print("\n" + "=" * 72)
print("   STACKING v9 COMPLETE")
print("=" * 72)
print(f"\n   Best global model: Stacking-L2  R²={global_results_v9['Stacking-L2']['r2']:.4f}")
print(f"   Delta vs v8            : {global_results_v9['Stacking-L2']['r2'] - RECORD_R2_GLOBAL:+.4f}")
print(f"  Speech Features impact : {delta_ab:+.4f} (on spoken segment)")
if global_results_v9["Stacking-L2"]["r2"] >= 0.45:
    print(f"\n   TARGET R² >= 0.45 REACHED!")
elif global_results_v9["Stacking-L2"]["r2"] >= 0.40:
    print(f"\n   Target R² >= 0.40 crossed!")
else:
    print(f"\n   Plateau: {global_results_v9['Stacking-L2']['r2']:.4f}")
    print(f"     Next direction: Multilingual BERT Transformer on subtitles")
